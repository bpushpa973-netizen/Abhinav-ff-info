...
async def GetAccountInformation(uid, region=None):
    try:
        print(f"📡 Requesting info for UID {uid}...")

        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.get(
                "http://187.127.175.208:5000/Bmw",
                params={"uid": uid}
            )
            ...
            result = resp.json()
            print(f"✅ Info received for UID {uid}")
            return result
...
def format_response(data):
    ...
    return {
        "AccountInfo": {
            ...
            "ShowBrRank": str(basic.get("show_br_rank", "0")),
            "ShowCsRank": str(basic.get("show_cs_rank", "0")),
...
<div id="welcomeBanner">
  <div class="banner-inner">
    ...
    <h1 class="banner-brand">ABHINAV-FF-INFO</h1>
...
<div id="mainSite" style="display:none;">
  ...
  <header>
    <div class="header-inner">
      <div class="logo-wrap">
        ...
        <span class="logo-text">ABHINAV<span class="logo-accent">xFF</span>INFO</span>
      </div>
  ...
  <footer>
    <p>⚡ ABHINAV-FF-INFO &copy; 2024 &nbsp;|&nbsp; Free Fire Player Info Tool &nbsp;|&nbsp; Made with ❤️</p>
  </footer>
...
(function initHack(){
  ...
  const chars  = 'アイウエオカキクケコ01アイウエオ10ABCDEF0123456789</>{}[];:#$%^&*|\\ABHINAV-FF-INFO'.split('');
...
def get_account_info():
    ...
    try:
        ...
        data = loop.run_until_complete(GetAccountInformation(uid))
        loop.close()
...
if __name__ == '__main__':
    print("="*55)
    print("🚀 ABHINAV-FF-INFO - Free Fire Info Website")
    print("="*55)
